...
| Flag | Required | Description |
| --- | --- | --- |
| `--force` |  | Release the lock even while a run is still active on the stack. Use when the run itself is stuck: the running Terraform keeps operating on state Ravion has stopped tracking for it, and its state write is discarded. |
| `--json` |  | emit response as JSON |
