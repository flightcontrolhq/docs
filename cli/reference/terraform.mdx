...
| Flag | Required | Description |
| --- | --- | --- |
| `--run-type <string>` | yes | The type of terraform run |
...
| `--drift-status <string>` |  | Drift status to filter by (optional, requires pipelineRunId) |
| `--json` |  | emit response as JSON |
| `--limit <int>` |  | limit |
| `--pipeline-run-id <string>` |  | Pipeline run ID to filter by (mutually exclusive with stackId) |
| `--run-type <string>` |  | Run type to filter by (optional, requires pipelineRunId) |
