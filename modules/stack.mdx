...
| Pipeline | ID                                |
| -------- | --------------------------------- |
| Change   | `pipe_system_tf_change_pipeline`  |
| Destroy  | `pipe_system_tf_destroy_pipeline` |
...
<Steps>
  <Step title="Create your pipeline">
    ...
    ravion pipeline config pull pipe_system_tf_change_pipeline --file change-pipeline.yaml
...
<Note>
  `ravion terraform state pull`, `list`, and `show` return a 404 for a managed workspace until the first state version is written. A workspace has no state until an apply runs, so a module created with `--initial-stack-run NONE` returns 404 until its first successful apply.
</Note>
