...
Every run reaches a final state, including runs that fail. A run that stays in progress and does not respond to cancellation indicates a problem beyond the stack itself. This is not expected behavior — [contact support](/contact-support) when you encounter it.
