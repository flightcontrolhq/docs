...
The module schema is too large to render on one page, so the sync splits it into pages of at most 150 fields each. It writes the parts to `snippets/schema-reference/module/`, the pages that import them to `module-definitions/definition-schema/`, a page index to `snippets/schema-reference/module-index.mdx`, and refreshes the "Definition schema" group in `docs.json`. All of those are generated — edit `scripts/sync-schema-reference.mjs` instead.
