...
A module instance is the unit you create, configure, and monitor. Each instance is created from a specific version of a **module definition** — the Ravion standard library (`rvn-*` types) or your own custom definitions — from your organization's **catalog**.
...
Ravion auto-creates default versions of these pipelines, and your platform team can customize or replace them — for example to add policy checks. The approval gate is skipped when a run is started with `autoapprove` (the `--autoapprove` CLI flag).
