...
<Update
  ...
  rss={{
    title: "EKS deployments, SOCI lazy loading, billing management, and new metrics and logs commands",
  }}
>
...
- **EKS deployments:** [Deploy EKS workloads and view their logs and metrics in Ravion](https://www.ravion.com/docs/module-definitions/definition-schema/deploy#eksmoduledeployment).
- **SOCI lazy loading:** Enable `soci_enabled` for Fargate images to use lazy-loaded container images ([build configuration](https://www.ravion.com/docs/pipelines/config-reference#buildimagecistep)).
...
- Build cache exports now use matching compression settings, avoiding unnecessary layer recompression.
- The in-app plan chooser now matches the pricing page, and Talk to sales links open the demo booking page.

### Fixes

- EC2 execution logs now include the user-facing AWS error when a launch fails.
- Billing contract rebuilds now use supported contract lookups and update entitlements successfully.
