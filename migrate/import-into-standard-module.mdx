...
<Steps>
  ...
  <Step title="Import into the managed workspace">
    ...
    <Warning>
      Always verify the state after importing — we've seen rapid back-to-back imports occasionally drop a write:

      ```bash
      ravion terraform state list --workspace <workspace>
      ```

      Every imported address must appear in the list. Re-run any import that's missing.
    </Warning>
