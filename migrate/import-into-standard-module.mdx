---
title: "Import existing resources into a standard module"
...
<Steps>
  ...
  <Step title="Match module inputs to the real resource">
    ...
    <Warning>
      Check `MasterUserSecret` before importing an RDS instance or Aurora cluster. If it is `null`, set **Manage password in Secrets Manager** to `false` before the post-import apply. Leaving it enabled asks AWS to generate a new master password and can disconnect every application still using the existing credentials. Leave the Terraform password unset to preserve an externally managed password; supplying it stores that value in Terraform state.
    </Warning>
...
- **Externally managed database passwords**: for RDS and Aurora resources where `MasterUserSecret` is `null`, disable **Manage password in Secrets Manager** and leave the password unset. The resulting configuration omits both password arguments, so AWS keeps the current password.
