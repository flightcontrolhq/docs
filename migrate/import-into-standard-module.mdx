...
<Steps>
  ...
  <Step title="Match module inputs to the real resource">
    ...
    <Warning>
      Check `MasterUserSecret` before importing an RDS instance or Aurora cluster. If it is `null`, set **Manage password in Secrets Manager** to `false` and **Preserve existing master password** to `true` before the post-import apply. Leaving password management enabled asks AWS to generate a new master password and can disconnect every application still using the existing credentials. The preservation confirmation keeps the Terraform password unset; supplying a password through advanced Terraform variables stores that value in Terraform state.
    </Warning>
...
- **Externally managed database passwords**: for RDS and Aurora resources where `MasterUserSecret` is `null`, disable **Manage password in Secrets Manager** and enable **Preserve existing master password**. The resulting configuration omits both password arguments, so AWS keeps the current password. The import-only confirmation is required so a fresh database cannot be created without credentials.
