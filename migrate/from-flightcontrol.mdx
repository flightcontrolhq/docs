...
<Note>
  We do not yet have any migration tooling or prompts. We plan to ship these and simple
  import/migration tooling for things like RDS, Redis, and CloudFront. These will follow in the coming weeks.

In the meantime, giving your agent the [Start here (human + agent)](../start-here) page and this migrate from Flightcontrol page should take you a long ways.
...
- Managed custom domain for ECS web services
  - Workaround is to create your own ACM certificate, validate it, then reference it in the ECS cluster module, then add the hostname routing rule in the ECS web module. See the [custom domains guide](/guides/custom-domains) for exact steps.
- Managed custom domain for static sites
  - Workaround is to create your own ACM certificate, validate it, then reference it in the static site module. See the [custom domains guide](/guides/custom-domains) for exact steps.
- RBAC
