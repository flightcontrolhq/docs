...
- Managed custom domain for ECS web services
  - Workaround is to create your own ACM certificate, validate it, then reference it in the ECS cluster module, then add the hostname routing rule in the ECS web module.
- Managed custom domain for static sites
  - Workaround is to create your own ACM certificate validate it, then reference it in the static site module
