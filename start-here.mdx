...
<Steps>
  ...
  <Step title="Use the project config file flow to create and manage modules">
    ...
    When the apply only creates new infrastructure, pass `--autoapprove` so the stack runs it starts apply without waiting at an approval gate. Omit it when the change modifies, replaces, or removes existing infrastructure.
