...
<Steps>
  ...
  <Step title="Connect your AWS account">
    Open [AWS accounts](https://app.ravion.com/org/settings/aws-accounts?connect=%7B%7D) and connect the AWS account where Ravion should provision infrastructure. Ravion deploys a CloudFormation stack that creates a cross-account IAM role.

    <Accordion title="Connect from the AWS CLI instead">
      Use this when you are already signed in to the AWS CLI for the account you want Ravion to manage. Confirm which account those credentials belong to first:
