...
test("canonical alias with noindex passes", () => {
  assert.deepEqual(
    checkCanonicalNoindex([alias, target], new Set(["concepts/modules"])),
    [],
  );
});
...
test("canonical alias without noindex fails", () => {
  ...
  assert.ok(
    violations.some((violation) => violation.includes("noindex: true")),
  );
});
...
test("a group root counts as navigated", () => {
  ...
  assert.deepEqual(
    checkCanonicalNoindex([alias, target], navigationPagePaths(navigation)),
    [],
  );
});
...
test("canonical target missing from navigation fails", () => {
  ...
  assert.ok(
    violations.some((violation) =>
      violation.includes("not in docs.json navigation"),
    ),
  );
});
...
test("canonical target with noindex fails", () => {
  ...
  assert.ok(
    violations.some((violation) => violation.includes("indexable and visible")),
  );
});
...
test("real repository content passes", () => {
  ...
  assert.deepEqual(
    checkCanonicalNoindex(
      readRepositoryPages(),
      navigationPagePaths(docsConfig.navigation),
    ),
    [],
  );
});
