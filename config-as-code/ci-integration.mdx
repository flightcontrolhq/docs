...
Set `RAVION_CLI_VERSION` to the [CLI release](/cli/releases) you want CI to use. Leave `RAVION_AUTOAPPROVE` as `"false"` to require manual approval of stack change pipeline runs in the Ravion dashboard, or set it to `"true"` to auto-approve project applies.
