...
5. Set `env.RAVION_CLI_VERSION` to the latest Ravion CLI version number, without the leading `v`. Find it from the latest release at https://github.com/flightcontrolhq/cli/releases/latest (the URL redirects to the newest tag, such as `v0.2.20` — use `0.2.20`). It must be `0.2.20` or newer, because the apply job passes `--description`. Or run `ravion --version` if the CLI is installed.
