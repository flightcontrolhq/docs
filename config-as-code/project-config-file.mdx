...
Any module input can take its value from another module's stack output with the `<< … >>` template syntax. Write the template as the input's value; there is no separate field or wrapper.
...
Address the producer relative to the consumer:
...
Use the `givenId` of each project, environment, and module. Templates can also combine outputs with literals, for example `<< "postgres://" + modules.db.output.host + ":5432" >>`.

How references behave:

- The template text is what is stored and what `ravion project config pull` exports, so the file round-trips unchanged and stays the source of truth.
- The value is resolved when the consumer's stack runs. `ravion project config apply` orders stack runs so producers run before the modules that reference them, and rejects reference cycles.
- If the producer has not been applied yet, the reference is accepted but stays pending; the input is validated against the resolved value once the producer's outputs exist. Apply the producer before running the consumer's stack.
...
- Sensitive outputs are not exposed through references. Reference a handle such as an ARN or secret name instead of the secret value.
- To write a literal `<<` in a value, escape it as `\<<`.

In the Ravion app, an input holding a template shows the template text as an editable text field with a note describing which module output it resolves from.
