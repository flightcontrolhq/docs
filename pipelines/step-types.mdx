---
title: "Step types"
description: "The catalog of available pipeline steps: module actions, CI steps, and control flow."
---
...
Builds a Docker image from a git source and pushes it to one or more ECR destinations. Build with a `dockerfile` or let `nixpacks` auto-detect the language and framework.
...
Builds static assets from a git source and uploads them to one or more S3 destinations. Build with a `dockerfile` or `nixpacks`.
